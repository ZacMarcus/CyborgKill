...
.defines(function(){
	...
	EntityWalker = ig.Entity.extend({
		...
		animSheet: new ig.AnimationSheet( 'media/walker.png', 19, 19 ),
		
		sfxHit: new ig.Sound('media/sounds/walker_hit.ogg'),
		sfxDie: new ig.Sound('media/sounds/walker_die.ogg'),
		...
		init: function( x, y, settings ) {
			...
			this.addAnim( 'hit', 0.5, [17] );
			this.addAnim( 'attack', 0.1, [24,25,26,27,28,29] );
			this.addAnim( 'die', 0.1, [16,17,18,19,19,19,19,20,21,22] );
			...
			if(!this.seenPlayer) {
				...
				if(this.currentAnim == this.anims.attack) {
					if( this.currentAnim.loopCount ) {
						...
						ig.game.spawnEntity(EntityWalkerProjectile, projectileSpawnX, this.pos.y+5, {flip:this.flip});
		...
		spawnParticles: function(count) {
			for(i = 0; i < count; i++) {
				...
				ig.game.spawnEntity( EntityWalkerParticle, x, y);
			}
		}
		
	});

	EntityWalkerProjectile = ig.Entity.extend ({
		size: {x: 6, y: 2},
		offset: {x: 0, y: 3},
		maxVel: {x: 100, y: 0},
		...
		animSheet: new ig.AnimationSheet( 'media/walker_projectile.png', 6, 7 ),
	...
	EntityWalkerParticle = EntityParticle.extend({
		lifetime: 3,
		fadetime: 2,
		bounciness: 0.6,
		vel: {x: 30, y: 20},
		...
		animSheet: new ig.AnimationSheet( 'media/walker_pieces.png', 4, 4 ),
